...
**说话人区分**（2026-06-12 修订）：场景改为**一台设备一个麦克风收多人**——用 **Deepgram 流式 diarization** 在单条音轨内按声纹区分说话人，匿名「说话人N」，不映射到具体人名。每台设备基本单一语种（中或日）；单麦中日混说为已知限制，暂不支持。

### 范围内（In scope）

- 双语实时字幕（中 ↔ 日），不做 TTS 语音同传
- 云 API 实现（LiveKit Cloud + Deepgram + LLM 翻译），不做自托管模型
- 一台设备一个麦克风收多人，单条音轨内按声纹 diarization 区分说话人（匿名编号）

### 范围外（Out of scope，YAGNI）

- TTS / 语音同传
- 声纹注册/识别到具体人名（仅匿名编号，不做 speaker enrollment）
- 单麦克风中日混合语种识别（每台设备假定单一语种）
...
// 定稿整句（带翻译，前端永久保留）
{
  ...
  "speaker": "Tang",         // 显示名
  "sid": "PA_xxx",           // 参与者 sid，前端按它分组到正确的人
  "srcLang": "zh",
